...
describe('parseQrPayload', () => {
  it('parses valid payload with device identity', () => {
    const raw = JSON.stringify({
      v: 1,
      d: { id: 'abc123', pub: 'pubkey', priv: 'privkey' },
      g: [{ u: 'wss://server:18789', t: 'token-abc', n: 'Home OC' }],
    });
    const result = parseQrPayload(raw);
    expect(result.v).toBe(1);
    expect(result.d?.id).toBe('abc123');
  ...
  it('rejects missing device identity', () => {
    ...
    expect(() => parseQrPayload(raw)).toThrow('missing device identity');
  });
  ...
  it('rejects missing gateways', () => {
    expect(() =>
      parseQrPayload(JSON.stringify({ v: 1, d: { id: 'abc123', pub: 'pubkey', priv: 'privkey' }, g: [] })),
    ).toThrow('no gateways');
  });
